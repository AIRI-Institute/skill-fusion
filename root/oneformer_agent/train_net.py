...
def hm3d2_validation_register():
  list_of_annotations = []
  dataset_dir = Path("datasets/hm3d2")
  annotation_dir = dataset_dir / "annotations_validation"
  images_dir = dataset_dir / "images_validation"
  for file in tqdm.tqdm(list(annotation_dir.iterdir())):
      list_of_annotations.append(
          {
            "file_name": images_dir / f"{file.name.split('.')[0]}.jpg",
            "height": 480,
            "width": 640,
            "image_id": f"{file.name.split('.')[0]}",
            "sem_seg_file_name": file.name,
          }
      )
  return list_of_annotations


DatasetCatalog.register("hm3d2_sem_seg_val", hm3d2_validation_register)

MetadataCatalog.get("hm3d2_sem_seg_val").stuff_classes = list(meta_clss.values())
MetadataCatalog.get("hm3d2_sem_seg_val").ignore_label = 255
MetadataCatalog.get("hm3d2_sem_seg_val").stuff_dataset_id_to_contiguous_id = {
    i: i
    for i in list(meta_clss.keys())
} 


def hm3d2_training_register():
  list_of_annotations = []
  dataset_dir = Path("datasets/hm3d2")
  annotation_dir = dataset_dir / "annotations_training"
...
DatasetCatalog.register("hm3d2_sem_seg_train", hm3d2_training_register)

MetadataCatalog.get("hm3d2_sem_seg_train").stuff_classes = list(meta_clss.values())
MetadataCatalog.get("hm3d2_sem_seg_train").ignore_label = 255
MetadataCatalog.get("hm3d2_sem_seg_traing").stuff_dataset_id_to_contiguous_id = {
    i: i
    for i in list(meta_clss.keys())
} 
MetadataCatalog.get("hm3d2_sem_seg_train").thing_dataset_id_to_contiguous_id = {
    i: i
    for i in list(meta_clss.keys())
} 
MetadataCatalog.get("hm3d2_sem_seg_train").thing_dataset_id_to_contiguous_id = {
