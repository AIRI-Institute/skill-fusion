...
from bps_nav_test.common.utils import Flatten, ResizeCenterCropper
from bps_nav_test.rl.ddppo.policy import regnetx, resnet
from bps_nav_test.rl.models.rnn_state_encoder import build_rnn_state_encoder
from bps_nav_test.rl.ppo import Net, Policy
from bps_nav_test.rl.ddppo.policy.resnet import FixupBasicBlock
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        self.model.eval()
        print('LOADED CLIP bps_nav_test.rl.ddppo.policy NO CROP')
        
        self.tr = transforms.Compose([
                    #Resize(self.model.visual.input_resolution, interpolation=PIL.Image.BICUBIC),
...
class ResNetNet(Net):
    ...
    def __init__(
        ...
        rnn_input_size = self._n_prev_action + 1024 + 512 # 768 for R50x16 CLIP               #######################
        ...
        if "gps" in observation_space.spaces:
            n_input_goal = 2
            self.gps_embeding = nn.Linear(n_input_goal, 32)
            rnn_input_size += 32
        else:
            self.gps_embeding = nn.Sequential()
            
        if "task_id" in observation_space.spaces:
            n_input_goal = 1
            self.taskid_embeding = nn.Linear(n_input_goal, 32)
            rnn_input_size += 32
        else:
            self.taskid_embeding = nn.Sequential()    
        ...
        if True:#not self.visual_encoder.is_blind:  #####################
    ...
    def forward(
        ...
        vis_priznaks = self.visual_forward(observations) ##################
        #print('VIZ PR SHAPE: ',vis_priznaks.shape)
        
        with torch.no_grad():
            clip_priznaks = observations["rgb"]#.permute(0,3,1,2)/255.
            #print('0 RGB SHAPE: ',clip_priznaks.shape,clip_priznaks.max(),clip_priznaks.min())
            clip_priznaks = self.visual_encoder.tr(clip_priznaks)
            clip_priznaks = self.visual_encoder.forward_clip(clip_priznaks)
        #print('CLIP_PRS: ',clip_priznaks.shape)
        
        #assert False, observations.keys()
        
        itog_proznaks = torch.cat((vis_priznaks,clip_priznaks), 1)
        
        #print('ITOG_PRS: ',itog_proznaks.shape)
    ...
    def rnn_forward(
        ...
        if task_id is not None:
            task_id = self.taskid_embeding(task_id)
            inputs.append(task_id)
            
        if gps is not None:
            gps = self.gps_embeding(gps)
            inputs.append(gps)    
