...
from bps_nav_test.common.utils import poll_checkpoint_folder
...
class BaseRLTrainer(BaseTrainer):
    ...
    def __init__(self, config):
        ...
        self.update = 0
        self.count_steps = 0
        self._prev_ckpt_percent = -1

        if config.NUM_UPDATES != -1 and config.TOTAL_NUM_STEPS != -1:
            raise RuntimeError("Cannot specify both NUM_UPDATES and TOTAL_NUM_STEPS")

        if config.NUM_UPDATES == -1 and config.TOTAL_NUM_STEPS == -1:
            raise RuntimeError("Must specify either NUM_UPDATES or TOTAL_NUM_STEPS")

    def percent_done(self):
        update = float(self.update)
        count_steps = float(self.count_steps)

        if self.config.TOTAL_NUM_STEPS != -1:
            return min(count_steps / self.config.TOTAL_NUM_STEPS, 1.0)
        else:
            return min(update / self.config.NUM_UPDATES, 1.0)

    def is_done(self):
        return self.percent_done() >= 1.0

    def should_checkpoint(self):
        checkpoint_every_percent = 1.0 / self.config.NUM_CHECKPOINTS

        percent_done = self.percent_done()
        should_checkpoint = (
            self._prev_ckpt_percent + checkpoint_every_percent
        ) <= percent_done

        if should_checkpoint:
            self._prev_ckpt_percent = percent_done
                ...
                while True:
                    ...
                    while current_ckpt is None:
                        ...
                        if (
                            ...
                            and os.path.basename(current_ckpt) == "ckpt.done.pth"
                        ):
                            return
