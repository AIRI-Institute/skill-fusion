...
from habitat.config.default_structured_configs import register_hydra_plugin, HeadingSensorConfig, TopDownMapMeasurementConfig,  HabitatSimSemanticSensorConfig
...
class HabitatRunner():
    def __init__(self):
        ...
        with habitat.config.read_write(task_config):
            ...
            task_config.habitat.simulator.agents.main_agent.sim_sensors.update(
    ...
    def run_episode(self, ii):
        ...
        save_dir = 'observations_dataset/episode_{}_{}'.format(ii, objectgoal_name)
        ...
        while not self.env.episode_over:
            ...
            action = pred['action']
            img_rgb = Image.fromarray(observations['rgb'].astype(np.uint8))
            img_rgb.save(f"{save_dir}/{step}_rgb.jpg")
            img_depth = Image.fromarray((observations['depth']*500).astype(np.uint16), 'I;16')
            ...
            img_sem = Image.fromarray(pred['pred'].astype(np.uint8)).convert('P')
            print(pred['pred'].astype(np.uint8).shape)
            print(np.unique(pred['pred'].astype(np.uint8)))
            img_sem.putpalette(np.array(meta_palete, dtype=np.uint8))
            img_sem.save(f"{save_dir}/{step}_pred_segmatron.png")
        ...
        """
        np.savetxt(os.path.join('observations_dataset/trajectories', 'episode_{}_{}.txt'.format(ii, objectgoal_name)), np.array(agent_trajectory))
        
        save_dir = 'fbe_poni_exploration_oneformer/episode_{}_{}_{}_{}'.format(ii, objectgoal_name, metrics['success'], round(metrics['spl'], 3))
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
...
def main():
    ...
    config = yaml.safe_load(fin)
    semantic_predictor = SemanticPredictor(config['semantic_predictor'])
    for dir in tqdm.tqdm(os.listdir("observations_dataset")):
        done = False
        for img in os.listdir(os.path.join("observations_dataset",dir)):
            if "segmatron_1_steps_delayed" in img:
                done = True
        if "trajec" in dir or "checkpoin" in dir or 'poses' in dir or 'angle' in dir:
            continue
        if not done:
            with open(os.path.join("observations_dataset","poses",f"{dir}.txt"), 'r') as fp:
                num_steps = len(fp.readlines())
            for step in range(0, num_steps):
                print(os.path.join("observations_dataset",dir,f"{step}_rgb.jpg"))
                frame = np.array(Image.open(os.path.join("observations_dataset",dir,f"{step}_rgb.jpg")))
                multiclass_prediction, semantic_mask, result = semantic_predictor(frame, objectgoal=1)
                img_sem = Image.fromarray(result.astype(np.uint8)).convert('P')
                print(result.astype(np.uint8).shape)
                print(np.unique(result.astype(np.uint8)))
                img_sem.putpalette(np.array(meta_palete, dtype=np.uint8))
                img_sem.save(f"observations_dataset/{dir}/{step}_pred_segmatron_1_steps_delayed.png")
