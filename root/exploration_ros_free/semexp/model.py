...
class Semantic_Mapping(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.erosion_size = args.erosion_size
        self.goal_decay = args.goal_decay
    ...
    def get_agent_view(self, depth, semantic):
        ...
        agent_view_t = du.transform_camera_view_t(
            point_cloud_t, self.agent_height, self.agent_view_angle, self.device
        )
    ...
    def forward(self, obs, pose_obs, pose_obs_old, maps_last, poses_last, old_poses_last, objectgoal_id):
        ...
        objgoal_map = agent_view_old[:, 4 + objectgoal_id, :, :]
        ...
        objgoal_map = torch.Tensor(np.expand_dims(np.expand_dims(objgoal_map, 0), 0)).to(self.device)
    ...
    def get_new_pose_batch(self, pose, rel_pose_change):
        #print('Pose:', pose)
        #print('Rel pose change:', rel_pose_change)
        pose[:, 1] += rel_pose_change[:, 0] * torch.sin(
            pose[:, 2] / 57.29577951308232
        ) + rel_pose_change[:, 1] * torch.cos(pose[:, 2] / 57.29577951308232)
        #print('Pose change 0 * sin:', rel_pose_change[:, 0] * torch.sin(pose[:, 2] / 57.29577951308232))
        #print('Pose change 1 * cos:', rel_pose_change[:, 1] * torch.cos(pose[:, 2] / 57.29577951308232))
