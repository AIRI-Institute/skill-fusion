...
class HabitatRunner():
    ...
    def run_episode(self, ii):
        ...
        if save:
            ...
            np.savetxt(os.path.join('observations_dataset/poses', 'episode_{}_{}.txt'.format(ii, objectgoal_name)), np.array(self.agent.robot_pose_track))
            np.savetxt(os.path.join('observations_dataset/view_angles', 'episode_{}_{}.txt'.format(ii, objectgoal_name)), np.array(view_angles))    
