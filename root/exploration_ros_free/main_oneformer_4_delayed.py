...
def main():
    fin = open('config_poni_exploration_4_steps_delayed.yaml', 'r')
    config = yaml.safe_load(fin)
    semantic_predictor = SemanticPredictor(config['semantic_predictor'])
    for dir in tqdm.tqdm(os.listdir("observations_dataset_15")):
        done = False
        for img in os.listdir(os.path.join("observations_dataset_15",dir)):
            if "segmatron_4_steps_delayed" in img:
                done = True
        if "trajec" in dir or "checkpoin" in dir or 'poses' in dir or 'angle' in dir:
            continue
        if not done:
            with open(os.path.join("observations_dataset_15","poses",f"{dir}.txt"), 'r') as fp:
                num_steps = len(fp.readlines())
            for step in range(0, num_steps):
                print(os.path.join("observations_dataset_15",dir,f"{step}_rgb.jpg"))
                frame = np.array(Image.open(os.path.join("observations_dataset_15",dir,f"{step}_rgb.jpg")))
                ...
                img_sem.putpalette(np.array(meta_palete, dtype=np.uint8))
                img_sem.save(f"observations_dataset_15/{dir}/{step}_pred_segmatron_4_steps_delayed.png")
