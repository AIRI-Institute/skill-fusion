...
class AStarPlanner:
    def __init__(self,
        ...
        self.vis_checker = LineOfSight()
        self.allow_diagonal = allow_diagonal
        self.unknown_cost = 10
        self.close_to_obstacle_cost = 10
    ...
    def findSuccessors(self, node):
        ...
        for di in range(-1, 2):
            for dj in range(-1, 2):
                ...
                if di == 0 and dj == 0:
                    continue
                if node.i + di < 0 or node.i + di >= self.map.shape[0]:#self.mapper.map_cells:
                    continue
                if node.j + dj < 0 or node.j + dj >= self.map.shape[1]:#self.mapper.map_cells:
                    continue
                if not self.vis_checker.checkTraversability(node.i + di, node.j + dj, self.map):
                    continue
    ...
    def smooth_path(self, path):
        ...
        for k in range(2, len(path)):
            i, j = path[k]
            if not self.vis_checker.checkLine(path_smoothed[-1][0], path_smoothed[-1][1],
                                              i, j,
                                              self.map):
    ...
    def computeCost(self, i1, j1, i2, j2):
        ...
        if self.map[i2, j2, 0] == 0 and self.map[i2, j2, 1] == 0:
            return dst * self.unknown_cost
        for di in range(-1, 2):
            for dj in range(-1, 2):
                if di == 0 and dj == 0:
                    continue
                if i2 + di < 0 or i2 + di >= self.map.shape[0]:#self.mapper.map_cells:
                    continue
                if j2 + dj < 0 or j2 + dj >= self.map.shape[1]:#self.mapper.map_cells:
                    continue
                if self.map[i2 + di, j2 + dj, 1] > 0:
                    return dst * self.close_to_obstacle_cost
        return dst


    #def create_path(self, observations, goal, unknown_is_obstacle=True, goal_reacher=False):
    def create_path(self, obst_map, start_i, start_j, goal_i, goal_j, unknown_is_obstacle=True, goal_reacher=False):
        ...
        start_node = Node(start_i, start_j)#self.getClosestNode(start_x, start_y)
        goal_node = Node(goal_i, goal_j)#self.getClosestNode(goal_x, goal_y)
        start_node.h = computeHFromCellToCell(start_node.i, start_node.j, goal_node.i, goal_node.j)
        #print('Start node i and j:', start_node.i, start_node.j)
        #print('Goal node i and j:', goal_node.i, goal_node.j)
        #print('Map around goal:', self.mapper.mapper.map[start_node.i - 5:start_node.i + 5, start_node.j - 5:start_node.j + 5, 1])
        start_node.f = self.hweight * start_node.h
        start_node.parent = None
        queue.add_to_open(start_node)
        path_found = False
        reach_distance = self.reach_radius# / self.mapper.args.map_resolution * 100.
        ...
        if path_found:
            ...
            #for k in range(len(path)):
                #i, j = path[k]
                #x, y = self.mapper.map_to_world(i, j)
                #path[k] = (x, y)
            return path
