    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            ...
            action_space=action_space,  # for action distribution
            policy_config=policy_config,
        )

    @classmethod
    def from_config(
        cls, config: DictConfig, observation_space: spaces.Dict, action_space
    ):
        return cls(
            observation_space=observation_space,
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        if not self.is_blind:
            ...
            print('!!num_compression_channels ',num_compression_channels,final_spatial,spatial_size,self.backbone.final_spatial_compress)
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        if self._n_input_depth > 0:
            depth_observations =  torch.nn.functional.dropout(observations["depth"], p=0.0, training=True, inplace=False)
            # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
            depth_observations = depth_observations.permute(0, 3, 1, 2)
            cnn_input.append(depth_observations)
            
        if self._n_input_semantic > 0:
            semantic_observations = torch.nn.functional.dropout(observations["semantic"], p=0.0, training=True, inplace=False)
            # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
...
class PointNavResNetNet(Net):
    ...
    def forward(
        ...
        return out, rnn_hidden_states, masks
